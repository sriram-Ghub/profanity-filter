...
module.exports = {
  ...
  clean: function (string) {
    ...
    for (key in dictionary) {
      ...
      while (index !== -1) {
		...
		return string; // break early to avoid external loop to finish, since we have a result
      }
    }
 ...
 /**
   * Clean the supplied whole word if in the internal dictionary
   * @method cleanWholeWord
   * @param  {String} string The phrase to be cleaned
   * @return {String}        The phrase with all words in the dictionary filtered
   */
  cleanWholeWord: function (string) {
    ...
    for (key in dictionary) {
      ...
      if (regex.test(string)) {
        keyReplacement = replacement[replacementMethod](key);
        string = string.replace(regex, keyReplacement);
      }
    }
